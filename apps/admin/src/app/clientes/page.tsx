...
export default function ClientesPage() {
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      ruc: cleanDoc,
      document_type,
      is_frequent: true, // Siempre caseros desde este módulo
    };
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-md overflow-hidden flex flex-col max-h-[85vh]">
            <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center shrink-0">
              <h2 className="text-xl font-bold text-gray-900">
                {editingCustomer ? 'Editar Cliente' : 'Nuevo Cliente Casero'}
              </h2>
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-gray-600 uppercase">TIPO DE DOCUMENTO</label>
                <select 
                  ...
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 bg-gray-50 font-medium text-gray-900 focus:outline-none focus:border-indigo-500"
                >
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-gray-600 uppercase">Nº Documento</label>
                <input 
                  type="text"
                  inputMode="numeric"
                  value={formData.ruc}
                  onChange={e => {
                    ...
                    const max = formData.document_type === "DNI" ? 8 : 11;
                    if (val.length <= max) setFormData({...formData, ruc: val});
                  }}
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 font-mono font-bold text-gray-900 focus:outline-none focus:border-indigo-500"
                  placeholder={formData.document_type === "DNI" ? "8 dígitos" : "11 dígitos"}
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-gray-600 uppercase">Razón Social / Nombre Completo</label>
                <input 
                  type="text"
                  value={formData.business_name}
                  onChange={e => setFormData({...formData, business_name: e.target.value})}
                  className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 font-bold text-gray-900 focus:outline-none focus:border-indigo-500 uppercase"
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50 shrink-0 flex gap-3">
              ...
              <button 
                onClick={handleSave}
                disabled={isSaving}
                className="flex-[2] py-2.5 rounded-xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-md shadow-indigo-600/20 transition-all flex items-center justify-center gap-2"
              >
                {isSaving ? "Guardando..." : <><CheckCircle2 className="w-5 h-5"/> Guardar Cliente</>}
              </button>
            </div>
          </div>
        </div>
