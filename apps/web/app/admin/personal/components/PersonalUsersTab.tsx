...
import {
  ...
  Search, Shield, AlertCircle, Crown
} from 'lucide-react';
          ...
          <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="px-5 py-3 bg-muted/10 border-b border-border flex flex-col gap-2.5 text-xs text-muted-foreground">
              ...
              <div className="pt-2 border-t border-border/50 flex flex-wrap items-center gap-x-6 gap-y-2">
                <span className="font-bold text-foreground">Ámbito de Acceso:</span>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-purple-600 bg-purple-50 px-2.5 py-0.5 rounded-full border border-purple-200">
                    Ámbito Global
                  </span>
                  <span className="font-medium text-muted-foreground">Aplica a todas las sucursales (según los permisos del rol)</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-0.5 rounded-full border border-blue-200">
                    Por Sucursal
                  </span>
                  <span className="font-medium text-muted-foreground">Restringido a las tiendas asignadas específicamente</span>
                </div>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {sortedProfiles.map((profile: any) => {
                      ...
                      const isOwnerUser = Boolean(profile.is_owner);
                      const isSelfUser = profile.id === currentProfileId || (profile.username && profile.username === currentUsername);
                      const canEditUser = isOwnerUser ? Boolean(isOwner) : checkCanManageTarget(profile, linkedEmp);
                      const canDeleteUser = !isOwnerUser && !isSelfUser && (profile.role === 'ADMIN' ? isOwner : checkCanManageTarget(profile, linkedEmp, { requireFullCoverage: true }));
                          ...
                          <td className="px-5 py-3.5 text-center">
                            <div className="flex items-center justify-center gap-2">
                              ...
                              {isDeleted ? (
                                ...
                                <>
                                  {Boolean(permissions?.personal_edit_user) && (!canEditUser ? null : (
                                    <button
                                  ...
                                  {Boolean(permissions?.personal_delete_user) && (!canDeleteUser ? null : (
                                    <button
                                      onClick={() => {
